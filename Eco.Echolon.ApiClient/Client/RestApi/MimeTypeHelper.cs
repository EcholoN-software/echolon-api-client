...
namespace Eco.Echolon.ApiClient.Client.RestApi
{
    public class MimeTypeHelper
    {
        ...
        private static IDictionary<string, string>? _fileTypes;

        protected static IDictionary<string, string> FileTypes
        {
            get
            {
                lock (Locker)
                {
                    _fileTypes ??= BuildFileExtensionDictionary();
                }
                return _fileTypes;
            }
        }

        public string GuessMimeTypeByFileName(string nameOrExtension)
        {
            var ext = Path.GetExtension(nameOrExtension);
            if (string.IsNullOrEmpty(ext))
                ext = nameOrExtension;
            if (ext.StartsWith("."))
                ext = ext.Substring(1);
            return FileTypes.TryGetValue(ext, out string result) ? result : "application/octet-stream";
        }

        private static IDictionary<string, string> BuildFileExtensionDictionary()
        {
            ...
            mimeTypes.Add("md", "text/markdown");
