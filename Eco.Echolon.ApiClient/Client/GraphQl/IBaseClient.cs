...
namespace Eco.Echolon.ApiClient.Client.GraphQl
{
    public interface IBaseClient
    {
        ...
        Task<GraphQlResponse<T>> Query<T>(string[] path, IDictionary<string, object?>? input = null,
            CancellationToken cancellationToken = default) where T : class;

        Task<GraphQlResponse<T>> Mutation<T>(string[] path, IDictionary<string, object?>? input = null,
            CancellationToken cancellationToken = default) where T : class;
    }
}
